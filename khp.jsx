...
try{
	...
	nodeStack.push(new CCNode());
	mainExportLoop(tempDocument);
	rootNode = nodeStack.pop();

	saveFile(sceneName + ".kju",JSON.stringify(rootNode));
}catch(e){
	alert("line : " + e.line + "\n" + e.message);
}finally{
...
function CCNode(){
	...
	this.parseAttr = function(attrArr){
...
function CCSprite(){
	CCNode.call(this);
	this.nodeType = "CCSprite";
	this.file = "";
}

function CCLayer(){
	CCNode.call(this);
	this.nodeType = "CCLayer";
}

function CCLabelTTF(){
	CCNode.call(this);
	this.nodeType = "CCLabelTTF";
	this.text = "";
}

function CCControlButton(){
	CCNode.call(this);
	this.nodeType = "CCControlButton";
	this.file = "";
}
